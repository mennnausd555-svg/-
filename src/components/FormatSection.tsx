...
export default function FormatSection({ isEnglish, user, config, onSelectElement }: FormatSectionProps) {
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-8">
        <div className="flex items-start gap-6">
          <div className="w-16 h-16 bg-brand-primary/10 rounded-2xl flex items-center justify-center shrink-0 border border-brand-primary/20 shadow-[0_0_20px_rgba(0,102,255,0.1)]">
            <LayoutTemplate className="w-8 h-8 text-brand-primary" />
          </div>
          <div>
            <h2 
              {...getEditableProps('pages.formats.title', 'text', 'Formats Page Title')}
              className="text-4xl font-black text-white tracking-tight mb-3"
            >
              {config?.pages?.formats?.title || t.formats}
            </h2>
            <p 
              {...getEditableProps('pages.formats.subtitle', 'text', 'Formats Page Subtitle')}
              className="text-dim text-xl max-w-2xl leading-relaxed font-medium"
            >
              {config?.pages?.formats?.subtitle || (isEnglish 
                ? 'Choose from 15+ battle-tested viral blueprints to craft scripts that dominate the narrative.' 
                : 'اختر من بين 15 فورمات مثبتة النجاح لكتابة اسكربتات تحقق ملايين المشاهدات.')}
            </p>
          </div>
        </div>
        <div className="flex items-center gap-4 px-6 py-3 bg-white/5 rounded-2xl border border-white/5">
          ...
          <span className="text-sm font-black uppercase tracking-widest text-dim">
            {isEnglish ? 'Premium Engine Active' : 'المحرك المميز نشط'}
          </span>
        </div>
      </div>
      
      <ScriptForm isFormatMode={true} onSubmit={handleSubmit} isLoading={isLoading} isEnglish={isEnglish} />
