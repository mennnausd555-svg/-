...
import { ScriptHistory, ScriptResult } from '../types';
...
export default function ScriptHistoryView() {
 ...
 if (loading) return <div className="flex justify-center py-12 text-[var(--text-secondary)]">جاري التحميل...</div>;

 return (
 <div className="space-y-6 animate-fade-up">
 <div className="flex flex-col md:flex-row justify-between items-center gap-4">
 <h2 className="text-2xl font-bold text-[var(--text-primary)] tracking-tight">سجل الاسكربتات</h2>
 ...
 className={`w-full text-right p-4 card-modern transition-all hover:shadow-md ${selectedScript?.id === item.id ? 'border-[#EFFF04] ring-2 ring-[#EFFF04]/10 ' : ''}`}
 >
 ...
 <p className="text-sm text-[var(--text-secondary)] font-medium">{new Date(selectedScript.created_at).toLocaleString('ar-EG')}</p>
 </div>
 ...
 {result.analysis ? (
   <div className="space-y-4">
     <div className="bg-[#EFFF04]/10/50 p-5 rounded-xl border border-[#EFFF04]/20 text-xs leading-relaxed text-[var(--text-secondary)]">
       <p className="font-bold text-[#EFFF04] mb-3 flex items-center gap-1.5 text-sm">
         <Video className="w-4 h-4" /> التحليل الشامل والمشاهد المقترحة
       </p>
   ...
   <div className="space-y-4">
     <div className="bg-[#EFFF04]/10/50 p-5 rounded-xl border border-[#EFFF04]/20 text-xs leading-relaxed text-[var(--text-secondary)]">
       <p className="font-bold text-[#EFFF04] mb-3 flex items-center gap-1.5 text-sm">
         <Video className="w-4 h-4" /> تحليل المشاهد
       </p>
       {result.sceneAnalysis}
     </div>
   </div>
 ...
 <p className="font-medium">اختر اسكربت من القائمة لعرض تفاصيله</p>
 </div>
 )}
 </AnimatePresence>
 </div>
